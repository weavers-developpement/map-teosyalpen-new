...
//Onload slide_1 text display
    ...
    $("#bh-sl-address").val("");
    $('#map-container').storeLocator("destroy");
  ...
  if (isMobile.matches) {
    $('#fullpage').fullpage ({
      ...
      menu: '#navSlide',
        ...
        // anchors: ['home', 'natural','gentle_injection','momdocteur','medias','teoxane'],
        anchors: ['home', 'natural','gentle_injection','medias','teoxane'],
      autoScrolling: true,
        scrollBar: false,
      css3: false,
   ...
   $rightControl.addClass('slideOnecontrol')
	...
	// if the last slide,
		if (!$active.next().length) {
			...
			$rightControl.hide();
			$leftControl.find('.enSlideTxt').append('<div class="preview txt3">INYECCIONES SUAVES</div>');
      ...
      sessionStorage.setItem('sliderindex', 3);
		// if not,
			...
			$rightControl.fadeIn();
			$rightControl.find('.enSlideTxt').append('<div class="preview txt3"> UNA HERRAMIENTA PRECISA </div>');
			$rightControl.find('.frSlideTxt').append('<div class="preview txt3 txt33"> Un dispositif de haute précision...</div>');
			$leftControl.find('.enSlideTxt').append('<div class="preview txt2">EL PRIMER SISTEMA</div>');
      ...
      sessionStorage.setItem('sliderindex', 1);
			...
			$leftControl.hide();
			$rightControl.find('.enSlideTxt').append('<div class="preview txt1">INYECCIONES SUAVES</div>');
...
function teoxaneheightset() {
	//alert('dad');
		...
		if (curvwidth >= 979 ) {
		//alert(curvwidth);
			var curvheightTop = parseInt($(window).height()  *  0.375*2);
			var curvheightBottom = $(window).height() - curvheightTop*2;
			$('.teoxaneTop').css('height', parseInt(curvheightTop));
